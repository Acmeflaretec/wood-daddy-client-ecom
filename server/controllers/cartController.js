...
const addCart = async (req, res) => {
  ...
  try {
    const existingCart = await Cart.findOne({ userId: _id, proId });
    if (existingCart) {
      await Cart.updateOne({ _id: existingCart._id }, { $inc: { qty: 1 } });
      return res
        .status(201)
        .json({
      ...
      res
        .status(201)
        .json({ message: "Cart entry created successfully", cart: newCart });
    }
  } catch (error) {
...
const removeCart = async (req, res) => {
  ...
  try {
    ...
    if (!removedCart) {
      return res
        .status(404)
        .json({ error: "Cart item with the specified proId not found" });
    }
...
const increasecartqty = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error incrementing cart quantity", error });
  }
};
