...
import { useEditAds, useGetAdsById, useDeleteAds } from "queries/ProductQuery";

const EditAds = () => {
   const { id } = useParams();
   const navigate = useNavigate()
   const { data: res, isLoading } = useGetAdsById({ id });
   useEffect(() => {
      setData(res?.data)
   }, [res])
  ...
  const handleFileChange = (event) => {
    ...
    setData(prev => ({ ...prev, imgUrl: file }));
  };

   const handleChange = (e) => {
      setData(prev => ({ ...prev, [e.target.name]: e.target.value }));
   };
   const { mutateAsync: editAds, isLoading: updating } = useEditAds()
   const { mutateAsync: deleteAds, isLoading: deleting } = useDeleteAds()
      ...
      try {
         ...
         if (!data?.title) {
          ...
          for (const key in data) {
            if (data.hasOwnProperty(key) && key !== "imgUrl") {
              formData.append(key, data[key]);
            }
          }
    
          typeof (data.imgUrl) == 'object' && formData.append("advertisement", data.imgUrl, data?.imgUrl?.name);
         editAds(formData)
            .then((res) => {
               if (res) {
                  ...
                  >
                     {data?.imgUrl ? (
                        <img
                           ...
                           src={typeof (data?.imgUrl) == 'object' ? URL.createObjectURL(data.imgUrl) : `${process.env.REACT_APP_API_URL}/uploads/${data.imgUrl}`}
                        />
                     ) : (
                        <React.Fragment>
                           <svg
